##Analysis of TPP macophages ChIPseq data (H3k27ac) fron HiSeq 4000 run (50bp SE reads) with hg19.
...
#Below is the code used to analyse the biological replicates merged based on their genotype at rs2836882.
#The homozygotes(hom) minor or major and the heterozygotes (het) samples are merged using the bam files as indicated above.
...
#need to index new merged files:
for a in *or_merged.bam; do samtools index $a; done
for a in *HET_merged.bam; do samtools index $a; done
#checked if downsample was necessary (cf. code from the ChIPseq_pipeline) but no need to downsample here.

#Run Macs2 with merged biological replicates and inputs:
...
#run ROSE with these MACS2 peaks:
